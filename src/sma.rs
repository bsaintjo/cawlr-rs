...
impl SmaOptions {
    ...
    //         // Score will be None if a) No data at that position, or b) Position
    // kmer didn't         // contain motif of interest
    //         let score =
    //             scores[col_idx - 1].filter(|s| self.motifs.iter().any(|m|
    // m.within_kmer(s.kmer())));
    ...
    //             Some(&signal_score) => prev_max +
    // self.pos_bkde.pmf_from_score(signal_score).ln(),             None =>
    // prev_max,         };
    //         *first = val;
    ...
    //                 Some(&signal_score) => {
    //                     nuc_prev +
    // self.neg_bkde.pmf_from_score(signal_score).ln()                 }
    //                 None => prev_max,
    //             };
    //             *next = val;
    //         }
...
// impl SmaMatrix {
//     pub fn new(val_matrix: DMatrix<f64>, ptr_matrix: DMatrix<Option<usize>>)
// -> Self {         Self {
//             val_matrix,
//             ptr_matrix,
//         }
...
//     // TODO Make initial value 10/157 for linker, 1/157 for nucleosome
// positions     pub fn from_read(read: &ScoredRead) -> Self {
//         let mut val_dm = DMatrix::from_element(147usize, read.np_length() as
// usize + 1, f64::MIN);         val_dm.column_mut(0).fill((1. /
// 147.0f64).ln());

//         let ptr_dm = DMatrix::from_element(147, read.np_length() as usize +
// 1, None);         Self::new(val_dm, ptr_dm)
//     }
...
//     /// Use the pointer matrix to infer the most likley sequence of states
// from     /// the probability matrix.
//     ///
//     /// First use the probability matrix to find the most likely state it
// ended     /// with. Then use that index walk through the pointer matrix and
// get     /// each state and what was the state used to update the probability
// matrix.     pub fn backtrack(&self) -> VecDeque<States> {
//         use States::*;
...
// /// let states = vec![Linker, Linker, Linker, Nucleosome, Nucleosome,
// Linker]; /// let rle = states_to_rle(&states);
...
// impl<'a> SmaOutput<'a> {
//     fn new<M: MetadataExt>(metadata: &'a M, states_rle: Vec<(States, u64)>)
// -> Self {         Self {
...
//     // Converts the RLE States list into two vectors, one containing the
// starts of     // the nucleosome positions, the other of their lengths
...
//             f,
//             
// "{0}\t{1}\t{2}\t{3}\t0\t{strand_punc}\t{bed_start}\t{bed_end}\t{rgb}\
// t{block_count}\t{lengths}\t{starts}",             self.chrom(),
//             self.start_0b(),
//             self.end_1b_excl(),
//             self.name(),
//         )
